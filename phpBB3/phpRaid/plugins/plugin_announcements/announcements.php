...
if(empty($option) || $option == 'com_frontpage' || $option == '') {
	...
	while($data = $db_raid->fetch()) {
		...
		if($pMain->checkPerm('edit_announcements_any') || ($pMain->checkPerm('edit_announcements_own') && $data['profile_id'] == $pMain->getProfileID())) {
			$actions = '<a class="btn btn--sectionHeader--action btn--bg" href="index.php?option=com_announcements&amp;task=edit&amp;id='.$data['announcement_id'].'"><i class="material-icons md-14">edit</i></a>';
		} else {
			$actions = '';
		}
		...
		array_push($a_data,
			array(
				...
				'actions'=>$actions,
				'titleblock'=> $pLang['announcements']
			)
		);
	}
