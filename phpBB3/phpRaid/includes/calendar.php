	...
	function render(){
		...
		for($count=0;$count<=$dayCounter;$count++) {
			...
			if ($count<$firstDayOffset) {
				...
				//$dayname = date('D',mktime(0,0,0,$this->_month,$day,$this->_year));
				$dayname = date('N',mktime(0,0,0,$this->_month,$day,$this->_year));
				if ($dayname==1) {$dayname='Lun';}	
				else if ($dayname==2) {$dayname='Mar';}
				else if ($dayname==3) {$dayname='Mer';}
				else if ($dayname==4) {$dayname='Jeu';}
				else if ($dayname==5) {$dayname='Ven';}
				else if ($dayname==6) {$dayname='Sam';}
				else {$dayname='Dim';}
				array_push($week,array('empty'=>false,'day'=>$day,'dayname'=>$dayname,'isToday'=>(($day==$today)?true:false),'wday'=>$weekDayNr % 7,'events'=>$this->getEvents($day,$this->_month,$this->_year)));			
				$weekDayNr++;
			}
		}
		return $period;
