...
class ReportList
{
	...
	* @access public
	*/
    function ReportList($strMainAttributes = '', $strEvenRowAttributes = '', $strOddRowAttributes = '', $strHoverRowAttributes = '')
    {
	...
	* @access public
	*/
    function addOutputColumn($strFieldName, $strColumnTitle, $strColumnHeadClass, $strColumnClass, $strFormat = null, $strAlign = null, $intLine = null, $strFieldLink = '__NOLINK__', $strLinkDesc = null)
    {
		...
		$arrColumn['colheadclass'] = $strColumnHeadClass;
		$arrColumn['colclass'] = $strColumnClass;
	...
	*/
    function getListFromArray($arrRecords, $blnDoForDownload = false, $strOutputMethod = null)
    {
        ...
        if ($intTotalRecords == 0) {
			...
			$strList .= '<table class="table-section">';
			$strList .= '<thead><tr><th><span class="text-warning">'.$pLang['no_data'].'</span></th></tr></thead>';
			$strList .= '</table>';
            return($strList);
        }
		...
		if ($this->_arrTitleInfo['blnShowRecordCount']) {
			...
			if (count($this->_arrColumnInfo['arrSectionKeys']) == 0) {
				$strCaption .= 'Record(s) '.($intFirstRecord + 1)." to $intLastRecord of $intTotalRecords total.";
			}
		}            
		$strList .= $this->_makeListCaption($strCaption, $strOutputMethod, $intNumCols);

        // Show the column headings (titles) if allowed
        if ($this->_arrColumnInfo['blnShowColumnNames']) {
            $strRow = '';
            foreach ($arrColumnsToUse as $arrCol) {
                // Handle cell alignment if necessary
                if ($arrCol['colheadclass'] != '') {
                    $strColclass = $arrCol['colheadclass'];
					$strRow .= '<th class="'.$strColclass.'">';
                } else { 
					$strRow .= '<th>';
					}
                ...
                if ($this->_arrSortInfo['blnAllowSort'] && !($blnDoForDownload) && $arrCol['name']!='edit') {
                	if ($arrCol['name'] == $this->_arrSortInfo['strSortField'] && !($this->_arrSortInfo['blnSortDescending'])) {
         				if ($this->_arrSortInfo['blnUsePathParams'] == 1) {
							$icon = '<i class="material-icons md-20">arrow_drop_up</i>';
							if($strColTitle == '')
								$icon = '';

							if(substr($strColTitle,0,4) != '<inp' && substr($strColTitle,0,4) !='<lab')
		                		$strColTitle = $strColTitle.' <a class="btn btn--table-header btn--bg-sort" href="'.$this->_arrSortInfo['strSortURL'].urlencode($arrCol['name']).'/SortDescending-1">'.$icon.'</a>';
						} else {
							$icon = '<i class="material-icons md-20">arrow_drop_down</i>';
							if($strColTitle == '' || substr($strColTitle,0,4) == '<img')
								$icon = '';

							if(substr($strColTitle,0,4) != '<img' && substr($strColTitle,0,4) != '<inp' && substr($strColTitle,0,4) !='<lab')
			                	$strColTitle = $strColTitle.' <a class="btn btn--table-header btn--bg-sort" href="'.$this->_arrSortInfo['strSortURL'].urlencode($arrCol['name']).'&SortDescending=1'.$this->_arrSortInfo['strSortAnchor'].'">'.$icon.'</a>';
						}
                	} else {
						$icon = '<i class="material-icons md-20">arrow_drop_up</i>';
						if($strColTitle == '' || substr($strColTitle,0,4) == '<img')
						...
						if(substr($strColTitle,0,4) != '<img' && substr($strColTitle,0,4) != '<inp' && substr($strColTitle,0,4) !='<lab')
	                		$strColTitle = $strColTitle.' <a class="btn btn--table-header btn--bg-sort" href="'.$this->_arrSortInfo['strSortURL'].urlencode($arrCol['name']).$this->_arrSortInfo['strSortAnchor'].'">'.$icon.'</a>';
                	}
                }

                $strRow .= $strColTitle.'</th>';
            }
        ...
        for ($iRow = $intFirstRecord; $iRow < $intLastRecord; $iRow++) {
            ...
            foreach ($arrColumnsToUse as $arrCol) {
				...
				if (trim($arrCol['colclass']) != '') {
					$strRow .= '<td class="'.$arrCol['colclass'].'">';
				} else { 
					$strRow .= '<td>'; }
        ...
        if ($this->_arrPagingInfo['blnAllowPaging']) {
            $strList .= $this->_makePageNavigators($intTotalRecords, $intNumCols);
        }

        // Finish up our list
        $strList .= $this->_makeListFooter($strOutputMethod, $intNumCols);
	...
	*/
    ...
    {
        if ($strRow == '') { return ''; }
        return '<thead><tr '.$this->_arrTitleInfo['strFieldHeadingAttributes'].">$strRow</tr></thead><tbody>\r\n";
	...
	* @access private
	*/
	function _makeListCaption($strCaption, $strOutputMethod = 'html', $intNumListColumns)
    {
        if ($strCaption == '') { return(''); }
		//return "<thead><tr class=\"tr--paginationCount hidden-sm-down\"><th colspan=\"$intNumListColumns\"><span class=\"tag tag-number\">".$strCaption."</span></tr></thead>";
		return "<thead><tr class=\"tr--paginationCount hidden-sm-down\"><th colspan=\"$intNumListColumns\">".$strCaption."</tr></thead>";
    }
	...
	* @return string The included closing value for this output method
	* @access private
	*/
    function _makeListFooter($strOutputMethod = 'html', $intNumListColumns)
    {
	   	return("</tbody></table>");
    }
    ...
    *	No more than 7 pages will be allowed at any time, and the current
	...
	*/
    function _makePageNavigators($intTotalRecords, $intNumListColumns)
    {
        ...
        if ($intTotalRecords <= $intNumRecords || $intNumRecords <= 0) {
			...
			$strResult = "<thead><tr class=\"tr--pagination\"><th class=\"text-center\" colspan=\"$intNumListColumns\"></th></tr></thead>";
			$strResult = "";
            return($strResult);
        }

        $intNumPagesNeeded = ceil($intTotalRecords / $intNumRecords);
        $strRow = "<thead><tr class=\"tr--pagination\"><th class=\"text-center\" colspan=\"$intNumListColumns\"><nav aria-label=\"...\"><ul class=\"pagination justify-content-center\">";
		

        // What's the base for this SET of records?
        $intSetBaseRecord = 0;
        for ($iPage = 1; $iPage < $intNumPagesNeeded; $iPage++) {
            if (($iPage * 7 * $intNumRecords) > $intStartRecord) {
                // Found our page set
                $intSetBaseRecord = ($iPage - 1) * 7 * $intNumRecords;
                break;
            }
        }

        if ($intNumPagesNeeded > 7) {
            if ($intSetBaseRecord > 0) {
                $blnNeedLessButton = true;
            } else {
                $blnNeedLessButton = false;
            }
            if ($intSetBaseRecord < ($intTotalRecords - 7 * $intNumRecords)) {
        ...
        if ($intStartRecord > 0) {
            ...
            $strLink = '<li class="page-item"><a class="page-link page-link-custom" href="'.$this->_arrPagingInfo['strPagingURL'].$intBase.$strSortOptions.'">';
            $strRow .= "$strLink".'<i class="material-icons md-16">chevron_left</i></a></li> ';

            if ($blnNeedLessButton) {
                // Show the Back option to move to a different set of pages
                $intBase = $intSetBaseRecord - (7 * $intNumRecords);
                $strLink = '<li class="page-item"><a class="page-link page-link-custom" href="'.$this->_arrPagingInfo['strPagingURL'].$intBase.$strSortOptions.'">';
            ...
            $strRow .= '<li class="page-item disabled"><a class="page-link page-link-custom" href="#"><i class="material-icons md-14">chevron_left</i></a></li>';
            $strRow .= '';
        }

        // Show the regular page count navigators
        for ($iPage = 0; $iPage < $intNumPagesNeeded; $iPage++) {
            if (($iPage * $intNumRecords) >= ($intSetBaseRecord + 7 * $intNumRecords)){
                break;
            }
            ...
            if (($iPage * $intNumRecords) >= $intSetBaseRecord) {
                if (($iPage * $intNumRecords) == $intStartRecord) {
                    ...
                    $strRow .= '<li class="page-item active"><a class="page-link page-link-custom" href="#">'.($iPage + 1).'</a></li>';
                } else {
        ...
        if ($intStartRecord < ($intTotalRecords - $intNumRecords)) {
            ...
            $intBase = $intStartRecord + $intNumRecords;
            $strLink = '<li class="page-item"><a class="page-link page-link-custom" href="'.$this->_arrPagingInfo['strPagingURL'].$intBase.$strSortOptions.'">';
            $strRow .= $strLink.'<i class="material-icons md-14">chevron_right</i></a></li>';
        } else {
            $strRow .= '';
            $strRow .= '<li class="page-item disabled"><a class="page-link page-link-custom" href="#"><i class="material-icons md-14">chevron_right</i></a></li>';
        }

		$strResult = $this->_makeListTitles($strRow.'</ul></nav></th></tr></thead>');
		$strResult = str_replace('<tr '.$this->_arrTitleInfo['strFieldHeadingAttributes'].'>', '', $strResult);
		return($strResult);
    }
