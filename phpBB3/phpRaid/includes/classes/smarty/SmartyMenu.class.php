...
/**
 * Project:     SmartyMenu: CSS Menus for the Smarty Template Engine for custom template
...
class SmartyMenu {
    ...
    function saveMenu($name, $menu) {
        $_SESSION['SMARTYMENUSC'][$name] = $menu;
    }
    ...
    function loadMenu($name) {
        return isset($_SESSION['SMARTYMENUCUSTOM'][$name]) ? $_SESSION['SMARTYMENUCUSTOM'][$name] : false;
    }    
    ...
    function resetMenu($name) {
        unset($_SESSION['SMARTYMENUCUSTOM'][$name]);
    }
     ...
     * @param string $link the link text
     */
    function setItemAvatar(&$item, $avatar) {
        $item['avatar'] = $avatar;
    }
