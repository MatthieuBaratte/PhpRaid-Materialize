...
function timeDiff($timestamp,$detailed=false, $max_detail_levels=8, $precision_level='second'){
	...
	if(empty($time)) {
		...
		if($action == $pLang['away']) {
			$upcoming_raid = $pLang['upcoming_raid'].$time;
			$upcoming_raid = substr($upcoming_raid, 0, strlen($upcoming_raid)-2).$pLang['end_raid'];
			$upcoming_raid = str_replace("heures, ","heures et ",$upcoming_raid);
			return $upcoming_raid;
		}
		else
			return $pLang['old_raid'];
	}
}
