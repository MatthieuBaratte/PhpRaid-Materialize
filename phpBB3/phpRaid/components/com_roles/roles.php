...
if(empty($task) || $task == '') {
	...
	while($data = $db_raid->fetch()) {
		// admin options
		$admin = '<a class="btn btn--table-edit btn--outline" href="index.php?option='.$option.'&task=edit&id='.$data['role_id'].'"><i class="material-icons md-12">edit</i></a> ';
		...
		array_push($roles,
			array(
				...
				'edit'=>$admin,
				'checkbox'=>'<label class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" id="select[]" name="select[]" value="'.$data['role_id'].'" aria-label="..."><span class="custom-control-indicator custom-checkcheckbox custom-checkcheckbox--border"></span></label>'
			)
		);
	}
	...
	$report->addOutputColumn('header_color', $pLang['header_color'],'text-center hidden-sm-down','text-center hidden-sm-down', 'null', 'null');
	$report->addOutputColumn('body_color', $pLang['body_color'],'text-center hidden-sm-down','text-center hidden-sm-down', 'null', 'null');
	$report->addOutputColumn('font_color', $pLang['font_color'],'text-center hidden-sm-down','text-center hidden-sm-down', 'null', 'null');
