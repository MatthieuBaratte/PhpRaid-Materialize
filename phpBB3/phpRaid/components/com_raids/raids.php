...
if(empty($task) || $task == '') {
	...
	$report->addOutputColumn('location', $pLang['location'],'text-center', 'text-left', null, 'null');
	$report->addOutputColumn('date', $pLang['date'],'text-center', 'text-center', null, 'null' );
	$report->addOutputColumn('invite_time', $pLang['invite_time'],'text-center hidden-sm-down', 'text-center hidden-sm-down', null, 'null');
	$report->addOutputColumn('start_time', $pLang['start_time'],'text-center hidden-sm-down', 'text-center hidden-sm-down', null, 'null');
	$report->addOutputColumn('officer', $pLang['officer'],'text-center hidden-sm-down', 'text-center hidden-sm-down', null, 'null');
	$report->addOutputColumn('edit','<i class="material-icons md-16">edit</i>','text-center','text-center','null','null');
	$report->addOutputColumn('checkbox', '<label class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" onClick="invertAll(this,this.form);" aria-label="..."><span class="custom-control-indicator custom-checkcheckbox custom-checkcheckbox--border"></span></label>','text-center', 'text-center', 'null', 'right');
	...
	if($pMain->checkPerm('edit_raids_any') || (($task == 'new' && $pMain->checkPerm('edit_raids_own')) || ($task == 'edit' && $pMain->checkPerm('edit_raids_own') && ($pMain->getProfileID() == getProfileFromTable('raid', 'raid_id', $id ))))) {
		...
		$p->assign(
			array(
				...
				'titleError' => $pLang['raTitle_error'],
				...
				'inviteHour' => $pLang['raInvite_hour'],
				'inviteMin' => $pLang['raInvite_min'],
				'startText' => $pLang['raStart_text'],
				'startHour' => $pLang['raStart_hour'],
				'startMin' => $pLang['raStart_min'],
				'freezeText' => $pLang['raFreeze_text'],
				'titleText' => $pLang['raTitle_text'],
			...
			if (is_array($files)) {
				...
				foreach($files as $value) {
					if($value == $old_data['icon_name'])
						...
						$selected = '';
					$icons .= '<option value="'.$value.'"'.$selected.' data-icon="games/'.$pConfig['game'].'/images/raids/'.$value.'">'.str_replace(strchr($value, '.'), "", $value).'</option>';
				}
				...
				while($data = $db_raid->fetch()) {
					array_push($l_data,
						array(
							...
							'name' => $data['role_id'].'_limit',
							'field' => $data['role_id'].'_limit',
							'value' => $_POST[$data['role_id'].'_limit'],
							'errortext' => sprintf($pLang['atNumeric_error_text'],$data['att_name'])
						)
					);
				}
	...
	pRedirect('index.php?option='.$option);
} else if($task == 'raidcancel') {
	
	$sql['UPDATE'] = 'raid';
	$sql['VALUES'] = array(
		'icon_name' => 'aRaid_Cancelled.png',
		'expired' => '1'
		);
	$sql['WHERE'] = 'raid_id='.$_GET['id'];
	$db_raid->set_query('update', $sql, __FILE__, __LINE__);
