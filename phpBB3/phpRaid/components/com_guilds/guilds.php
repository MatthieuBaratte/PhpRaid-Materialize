...
if(empty($task) || $task == '') {
	...
	while($data = $db_raid->fetch()) {
		// admin options
		$admin = '<a class="btn btn--table-edit btn--outline" href="index.php?option='.$option.'&task=edit&id='.$data['guild_id'].'"><i class="material-icons md-12">edit</i></a> ';
		...
		array_push($phpr_a,
			array(
				...
				'edit'=>$admin,
				'checkbox'=>'<label class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" id="select[]" name="select[]" value="'.$data['guild_id'].'" aria-label="..."><span class="custom-control-indicator custom-checkcheckbox custom-checkcheckbox--border"></span></label>'
			)
		);
	}
	...
	$report->addOutputColumn('tag', $pLang['tag'],'text-center','text-center', 'null', 'null');
	$report->addOutputColumn('master', $pLang['master'],'text-center','text-center', 'null', 'null');
	$report->addOutputColumn('edit','<i class="material-icons md-16">edit</i>','text-center','text-center','null','null');
	$report->addOutputColumn('checkbox', '<label class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" onClick="invertAll(this,this.form);" aria-label="..."><span class="custom-control-indicator custom-checkcheckbox custom-checkcheckbox--border"></span></label>','alignSCcenter','alignSCcenter', 'null', 'right');
