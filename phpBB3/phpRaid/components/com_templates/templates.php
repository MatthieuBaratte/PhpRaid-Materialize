...
if(empty($task) || $task == '') {
	...
	while($data = $db_raid->fetch()) {
		...
		array_push($phpr_a,
			array(
				...
				'checkbox'=>'<label class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" id="select[]" name="select[]" value="'.$data['raid_template_id'].'" aria-label="..."><span class="custom-control-indicator custom-checkcheckbox custom-checkcheckbox--border"></span></label>'
			)
		);
	}
	...
	$report->addOutputColumn('icon_name', $pLang['icon_name'],'text-center','text-left', 'null', 'null');
