...
if(empty($task) || $task == '') {
	...
	$p->assign(
		array(
			...
			'rssRaids' => '<span class="content--description"><a href="index.php?option=com_rss&task=raid">'.$pLang['rssRaids'].'</a></span><span class="content--rsslink">'
							. $pConfig['site_url'].'/index.php?option=com_rss&task=raid</span>',
			'rssAnnouncements' => '<span class="content--description"><a href="index.php?option=com_rss&task=announcements">'.$pLang['rssAnnouncements'].'</a></span><span class="content--rsslink">'
							. $pConfig['site_url'].'/index.php?option=com_rss&task=announcements</span>',
		)
	);
	...
	$output .= "<title>phpRaider ".$pLang['rssRaidsFor']." ".$pConfig['game']."</title>\n
				<description>phpRaider ".$pLang['rssRaidsFor']." ".$pConfig['game']."</description>\n
	...
	while($data = $db_raid->fetch()) {
		$output .= "<item>
					...
					<guid>".$pConfig['site_url']."index.php?option=com_view&amp;id=".$data['raid_id']."</guid>
					<pubDate>".newDate(DATE_RFC2822, $data['invite_time'],0)."</pubDate>
